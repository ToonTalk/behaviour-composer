...
package uk.ac.lkl.server.oauth.client;
...
import uk.ac.lkl.server.oauth.OAuth;
import uk.ac.lkl.server.oauth.OAuthAccessor;
import uk.ac.lkl.server.oauth.OAuthConsumer;
import uk.ac.lkl.server.oauth.OAuthException;
import uk.ac.lkl.server.oauth.OAuthMessage;
import uk.ac.lkl.server.oauth.OAuthProblemException;
import uk.ac.lkl.server.oauth.http.HttpClient;
import uk.ac.lkl.server.oauth.http.HttpMessage;
import uk.ac.lkl.server.oauth.http.HttpMessageDecoder;
import uk.ac.lkl.server.oauth.http.HttpResponseMessage;
...
public class OAuthClient {
    ...
    throws IOException, OAuthException, URISyntaxException {
        ...
        uk.ac.lkl.server.oauth.ParameterStyle style = (ps == null) ? uk.ac.lkl.server.oauth.ParameterStyle.BODY
                : Enum.valueOf(uk.ac.lkl.server.oauth.ParameterStyle.class, ps.toString());
     ...
     *             the HTTP response status code was not 200 (OK)
     */
    public OAuthMessage invoke(OAuthMessage request,  uk.ac.lkl.server.oauth.ParameterStyle style)
     ...
     */
    public OAuthResponseMessage access(OAuthMessage request,  uk.ac.lkl.server.oauth.ParameterStyle style) throws IOException {
     ...
     * 
     * @deprecated use uk.ac.lkl.server.oauth.ParameterStyle.
     */
    public static enum ParameterStyle {
        AUTHORIZATION_HEADER(uk.ac.lkl.server.oauth.ParameterStyle.AUTHORIZATION_HEADER),
        BODY                (uk.ac.lkl.server.oauth.ParameterStyle.BODY),
        QUERY_STRING        (uk.ac.lkl.server.oauth.ParameterStyle.QUERY_STRING);

        public uk.ac.lkl.server.oauth.ParameterStyle getReplacement() {
            return replacement;
        }

        private ParameterStyle(uk.ac.lkl.server.oauth.ParameterStyle replacement) {
            this.replacement = replacement;
        }

        private final uk.ac.lkl.server.oauth.ParameterStyle replacement;
    }
